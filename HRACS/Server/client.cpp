...
int main(int argc, char **argv)
{
    ...
    ModuleMetrics registration = registration_cs(host, port, identity, password);
    ModuleMetrics login = login_cs(host, port, identity, password);
    ...
    vector<ModuleMetrics> single_query_metrics;
    for (int count : query_counts)
        single_query_metrics.push_back(single_query_cs(host, port, max_files, count));

    vector<ModuleMetrics> batch_query_metrics;
    for (int count : batch_counts)
        batch_query_metrics.push_back(batch_query_cs(host, port, max_files, count));

    vector<ModuleMetrics> update_metrics;
    for (int count : update_counts)
        update_metrics.push_back(update_cs(host, port, max_files, count));

    vector<ReportRow> rows;
    rows.push_back({"Registration", "0", registration});
    rows.push_back({"Login", "0", login});
    for (size_t i = 0; i < upload_metrics.size(); ++i)
        rows.push_back({i == 0 ? "Upload" : "", file_number_for(upload_metrics[i]), upload_metrics[i]});
    for (size_t i = 0; i < single_query_metrics.size(); ++i)
        rows.push_back({i == 0 ? "SingleQuery" : "", file_number_for(single_query_metrics[i]), single_query_metrics[i]});
    for (size_t i = 0; i < batch_query_metrics.size(); ++i)
        rows.push_back({i == 0 ? "BatchQuery" : "", file_number_for(batch_query_metrics[i]), batch_query_metrics[i]});
    for (size_t i = 0; i < update_metrics.size(); ++i)
        rows.push_back({i == 0 ? "Update" : "", file_number_for(update_metrics[i]), update_metrics[i]});
