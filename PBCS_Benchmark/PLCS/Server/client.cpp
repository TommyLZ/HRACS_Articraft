...
static constexpr int PBCS_ITERATIONS = 1;
...
static void ensure_dataset(int max_files)
{
    ...
    {
        ensure_test_file(fs::path("../File/TestMultiple/Origin") / file_name(i), static_cast<unsigned char>(0x31 + i));
        ensure_test_file(fs::path("../File/Update/Origin") / file_name(i), static_cast<unsigned char>(0x91 + i));
    }
}
...
static void mk_to_key(const string &mk, CryptoPP::byte *key)
{
    if (mk.size() < 16)
        throw runtime_error("mk is too short");
    memcpy(key, mk.data(), 16);
}

static pair<string, string> encrypt_file_locally(const string &mk, const fs::path &plain_path,
...
static void decrypt_file_locally(const string &mk, const string &cipher, const string &iv_blob,
                                 const string &kind, int index)
{
    fs::path cipher_path = client_cipher_path_for(kind, index);
    fs::path iv_path = client_iv_path_for(kind, index);
...
{
    cout << "\n[PBCS Client] >>> " << name << " started" << endl;
}

static void phase_done(const string &name)
{
    cout << "[PBCS Client] <<< " << name << " finished" << endl;
}

static cs::TimedResponse harden(const string &host, int port, Client &client, const string &id, element_t &b)
{
    element_t a;
    element_init_G1(a, pairing);
    client.blindPassword(a);
    cs::TimedResponse response = cs::request_timed(host, port, "HARDEN", {{"id", id}, {"a", element_to_blob(a)}});
    blob_to_element(b, cs::require_field(response.message, "b"));
    element_clear(a);
    return response;
}

static LoginState login_for_take(const string &host, int port, string &psw, string &id, ModuleMetrics &metrics)
{
    Client client(psw, id);
    element_t b;
    element_init_G1(b, pairing);
    metrics.add_request(harden(host, port, client, id, b));

    LoginState state;
    client.pwdGen(b, state.cred_cs);
    cs::TimedResponse cs_auth = cs::request_timed(host, port, "CS_TAKE_AUTH", {{"id", id}, {"cred_cs", state.cred_cs}});
    metrics.add_request(cs_auth);
    state.s_id = cs::require_field(cs_auth.message, "s_id");
    state.r_id = cs::require_field(cs_auth.message, "r_id");

    client.loginToKS_Take(state.t, state.k_1, state.k_2, state.s_id, state.r_id);
    cs::TimedResponse ks_auth = cs::request_timed(host, port, "KS_TAKE_AUTH", {{"id", id}, {"t", state.t}});
    metrics.add_request(ks_auth);
    state.ct = cs::require_field(ks_auth.message, "ct");
    state.tag = cs::require_field(ks_auth.message, "tag");
    client.recover(state.mk, state.k_1, state.k_2, state.ct, state.tag);
...
static ModuleMetrics key_give_cs(const string &host, int port, string &psw, string &id)
{
    ...
    string r_id, s_id, cred_cs;
    client.loginToCS_Give(r_id, s_id, cred_cs, id, b);
    cs::TimedResponse cs_auth = cs::request_timed(host, port, "CS_GIVE_AUTH",
                                                  {{"id", id}, {"cred_cs", cred_cs}, {"r_id", r_id}});
    metrics.add_request(cs_auth);
    s_id = cs::require_field(cs_auth.message, "s_id");

    string t, k_1, k_2, ct, tag;
    client.loginToKS_Give(t, k_1, k_2, ct, tag, s_id, r_id);
    metrics.add_request(cs::request_timed(host, port, "KS_GIVE_AUTH",
                                          {{"id", id}, {"t", t}, {"ct", ct}, {"tag", tag}}));
    element_clear(b);
    auto end = chrono::high_resolution_clock::now();
    metrics.client_ms = chrono::duration<double, milli>(end - start).count() - metrics.rtt_ms;
    phase_done("KeyGive");
...
static ModuleMetrics key_take_cs(const string &host, int port, string &psw, string &id)
{
    ...
    auto start = chrono::high_resolution_clock::now();
    login_for_take(host, port, psw, id, metrics);
    auto end = chrono::high_resolution_clock::now();
    metrics.client_ms = chrono::duration<double, milli>(end - start).count() - metrics.rtt_ms;
    phase_done("KeyTake");
    return metrics;
...
static void upload_one_file(const string &host, int port, const string &id, const LoginState &state,
                            int index, ModuleMetrics &metrics)
{
    string name = file_name(index);
    auto encrypted = encrypt_file_locally(state.mk, fs::path("../File/TestMultiple/Origin") / name, "Upload", index);
    metrics.add_request(cs::request_timed(host, port, "UPLOAD",
                                          {{"id", id}, {"cred_cs", state.cred_cs}, {"index", to_string(index)}, {"name", name}, {"cipher", encrypted.first}, {"iv", encrypted.second}}));
}
...
static ModuleMetrics upload_single_cs(const string &host, int port, string &psw, string &id)
{
    ...
    auto start = chrono::high_resolution_clock::now();
    LoginState state = login_for_take(host, port, psw, id, metrics);
    upload_one_file(host, port, id, state, TEST_INDEX, metrics);
    auto end = chrono::high_resolution_clock::now();
    metrics.client_ms = chrono::duration<double, milli>(end - start).count() - metrics.rtt_ms;
    phase_done("Upload");
    return metrics;
                                              ...
                                              int max_files, int upload_step)
{
    ...
    auto start = chrono::high_resolution_clock::now();
    LoginState state = login_for_take(host, port, psw, id, running);
    ...
    {
        upload_one_file(host, port, id, state, index, running);
        if (next_checkpoint < checkpoints.size() && index == checkpoints[next_checkpoint])
        {
            ...
            cout << "[PBCS Client] Upload progress: " << index << "/" << max_files << " files" << endl;
            ++next_checkpoint;
        }
    }
    ...
    const function<void(int, const ModuleMetrics &)> &commit)
{
    ...
    LoginState state = login_for_take(host, port, psw, id, auth);
    ...
    for (int checkpoint : checkpoints)
    {
        ...
        for (; index <= checkpoint; ++index)
            upload_one_file(host, port, id, state, index, segment);
        ...
        snapshots.push_back(snapshot);
        cout << "[PBCS Client] Upload checkpoint committed: " << checkpoint
                                     ...
                                     int total_files, int operation_files)
{
    ...
    for (int index = 1; index <= operation_files; ++index)
    {
        cs::TimedResponse query = cs::request_timed(host, port, "SINGLE_QUERY",
                                                    {{"id", id}, {"cred_cs", state.cred_cs}, {"index", to_string(index)}});
        metrics.add_request(query);
        decrypt_file_locally(state.mk, cs::require_field(query.message, "cipher"),
                               ...
                               int total_files, int operation_files)
{
    ...
    for (int index = 1; index <= operation_files; ++index)
    {
        ...
        decrypt_file_locally(state.mk, cs::require_field(old_file.message, "cipher"),
                             cs::require_field(old_file.message, "iv"), "Recover(Update)", index);

        auto encrypted = encrypt_file_locally(state.mk, fs::path("../File/Update/Origin") / name, "Update", index);
...
static fs::path write_metrics_csv(const vector<ReportRow> &rows)
{
    ...
    ostringstream name;
    name << put_time(&local_tm, "%Y%m%d") << "_PBCS.csv";
...
int main(int argc, char **argv)
{
    ...
    int iterations = argc > 7 ? stoi(argv[7]) : PBCS_ITERATIONS;
    ...
    fs::path checkpoint_path = fs::path("../Result") /
                               ("PBCS_resume_" + to_string(max_files) + "_" + to_string(upload_step) + "_" +
    ...
    string checkpoint_config = "PBCS|" + host + "|" + to_string(port) + "|" + id + "|" +
    ...
    cout << "[PBCS Client] Connected target " << host << ":" << port << endl;
    cout << "[PBCS Client] Running " << iterations << " averaged rounds; max_files="
    ...
    if (!checkpoint.has("Registration", -1, 0))
    {
        ...
        cout << "[PBCS Client] Resume: Registration already completed" << endl;
    registration_total = checkpoint.get("Registration", -1, 0);

    if (!checkpoint.has("Setup", -1, 0))
    {
        key_give_cs(host, port, psw, id);
        checkpoint.put("Setup", -1, 0, ModuleMetrics{"Setup"});
    }
    else
        cout << "[PBCS Client] Resume: KeyGive already completed" << endl;

    if (!checkpoint.has("Login", 0, 0))
    {
        ModuleMetrics measured = login_cs(host, port, psw, id);
    ...
    if (first_missing_upload < static_cast<int>(upload_counts.size()))
    {
        ...
        cout << "[PBCS Client] Resume: Upload series already completed" << endl;

    for (int i = 0; i < iterations; ++i)
    {
        cout << "\n[PBCS Client] ===== Iteration " << (i + 1) << "/" << iterations << " =====" << endl;
        if (!checkpoint.has("Login", i, 0))
        {
            ...
            cout << "[PBCS Client] Resume: Login iteration " << (i + 1) << " completed" << endl;
        ...
        for (size_t j = 0; j < query_counts.size(); ++j)
        {
            if (!checkpoint.has("SingleQuery", i, query_counts[j]))
            {
                ...
                cout << "[PBCS Client] Resume: SingleQuery " << query_counts[j]
        ...
        for (size_t j = 0; j < batch_counts.size(); ++j)
        {
            if (!checkpoint.has("BatchQuery", i, batch_counts[j]))
            {
                ...
                cout << "[PBCS Client] Resume: BatchQuery " << batch_counts[j]
                     << " iteration " << (i + 1) << " completed" << endl;
        }
        for (size_t j = 0; j < update_counts.size(); ++j)
        {
            if (!checkpoint.has("Update", i, update_counts[j]))
            {
                ...
                cout << "[PBCS Client] Resume: Update " << update_counts[j]
                     << " iteration " << (i + 1) << " completed" << endl;
        }
    }
