...
const int PASSWORD_LEN_MIN = 6;
const int PASSWORD_LEN_MAX = 12;
const string CHARSET_FULL = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()-_=+[]{};:,.<>?";

int pick_password_length(const string &seed, uint64_t idx)
{
    string s = seed + "|LEN|" + to_string((long long)idx);
    auto h = sha256_bytes(sb(s));
    int range = PASSWORD_LEN_MAX - PASSWORD_LEN_MIN + 1;
    return PASSWORD_LEN_MIN + (int)(h[0] % range);
}
...
string generate_realistic_password_for_user(const SyntheticUser &u, uint64_t seed_idx, int target_len)
{
    string n = u.name;
    string b = u.birthday;
    ...
    int base_len = min(8, target_len);
    base = pad_or_truncate(base, base_len);
    string myseed = seed + "|" + u.name + "|" + u.birthday + "|" + u.phone;
    int tail_len = max(0, target_len - (int)base.size());
    string tail = deterministic_rand_string(myseed, seed_idx, tail_len, CHARSET_FULL);
    string pw = base + tail;
    vector<unsigned char> h = sha256_bytes(vector<unsigned char>(myseed.begin(), myseed.end()));
    for (int t = 0; t < 3 && (int)pw.size() > 0; ++t)
    {
        int pos = h[t] % (int)pw.size();
    ...
    return pad_or_truncate(pw, target_len);
}
...
void generate_mangled_variants_enhanced(const string &base, unordered_set<string> &outset, int &remaining)
{
    ...
    auto add_if_ok = [&](const string &s)
    {
        ...
        int len_t = pick_password_length("MANGLE_LEN|" + t, 0);
        t = pad_or_truncate(t, len_t);
...
pair<vector<string>, vector<int>> make_candidate_dict_pair(int dictsize, const string &leak_file = "", const string &popular_file = "popular_passwords.txt")
{
    ...
    if (!leak_file.empty() && file_exists(leak_file))
    {
        ...
        for (const auto &pw : leaked)
        {
            if (remaining <= 0)
                break;
            int len_i = pick_password_length("LEAK_LEN|" + pw, 0);
            string p = pad_or_truncate(pw, len_i);
    ...
    for (const auto &pw : popular)
    {
        if (remaining <= 0)
            break;
        int len_i = pick_password_length("POPULAR_LEN|" + pw, 0);
        string p = pad_or_truncate(pw, len_i);
        if (seen.insert(p).second)
        {
            out.push_back(p);
    ...
    while (remaining > 0)
    {
        int len_i = pick_password_length("DICT_FILL_LEN", seed_counter);
        string cand = deterministic_rand_string("DICT_FILL", seed_counter, len_i, CHARSET_FULL);
...
bool read_true_pw_file(const string &path, int M, vector<string> &out_pw)
{
    ...
    while (getline(ifs, line))
    {
        if (!line.empty())
        {
            ...
            out_pw.push_back(str_trim(line));
...
void ensure_unique_passwords(vector<string> &pw_vec, const string &uniq_seed = "UNIQUENESS")
{
    ...
    for (size_t i = 0; i < pw_vec.size(); ++i)
    {
        ...
        if (pw_vec[i].empty())
        {
            int len_i = pick_password_length("EMPTY_FALLBACK_LEN", (uint64_t)i);
            pw_vec[i] = pad_or_truncate("", len_i);
        }
        ...
        int len_i = (int)base.size();
        while (true)
        {
            int replace_len = min(6, len_i);
            string extra = deterministic_rand_string(uniq_seed + "|" + to_string((long long)i), trycnt, replace_len, CHARSET_FULL);
            string cand = base;
            cand.replace(len_i - replace_len, replace_len, extra.substr(0, replace_len));
            if (seen.insert(cand).second)
            {
                pw_vec[i] = cand;
                break;
...
int main(int argc, char *argv[])
{
    if (argc < 2)
    {
        cerr << "Usage: ./scheme1_est_varlen <pbc_param_file> [M] [dictsize] [time_per_match] [lock_seconds] [max_budget] [effective_cap]\n";
        return 1;
    }
    ...
    if ((int)dict.size() < dictsize)
    {
        ...
        {
            int len_i = pick_password_length("DICT_FILL_EXT_LEN", sc);
            string cand = deterministic_rand_string("DICT_FILL_EXT", sc++, len_i, CHARSET_FULL);
            dict.push_back(cand);
        }
    }

    cout << "# scheme1_pbc_incremental: param=" << param_file << ", M=" << M << ", dictsize=" << dictsize
         << ", mode=baseline, pw_len_range=[" << PASSWORD_LEN_MIN << "," << PASSWORD_LEN_MAX << "]\n";
    ...
    true_pw.resize(M);
    string true_pw_file = "true_pw_varlen_" + to_string(M) + ".txt";
    ...
    if (!read_ok)
    {
        ...
        for (int i = 0; i < M; ++i)
        {
            int len_i = pick_password_length("TRUE_PW_LEN", (uint64_t)i);
            if (!dict.empty() && i < TRUE_COUNT)
            {
                ...
                if (idx >= dict.size())
                    idx = dict.size() - 1;
                true_pw[i] = pad_or_truncate(dict[idx], len_i);
            }
            else
            {
                string seed = "FALLBACKPW|" + users[i].name + "|" + users[i].birthday + "|" + users[i].phone;
                true_pw[i] = deterministic_rand_string(seed, (uint64_t)i + 9999ULL, len_i, CHARSET_FULL);
            }
        }
        cout << "ths" << endl;
    ...
    {
        ofstream ofs("baseline_crackedpw_varlen_at.txt", ios::out | ios::trunc);
        for (int i = 0; i < M; ++i)
            ofs << cracked_at_attempt[i] << '\n';
    }
    cout << "# baseline per-user cracked attempts written to baseline_crackedpw_varlen_at.txt\n";
    ...
    {
        ofstream ofs("rate_limited_estimated_crackedpw_varlen_at.txt", ios::out | ios::trunc);
        for (int i = 0; i < M; ++i)
            ofs << effective_attempt[i] << '\n';
    }
    cout << "# estimated rate-limited per-user attempts written to rate_limited_estimated_crackedpw_varlen_at.txt\n";
    ...
    cout << "#  - baseline_crackedpw_varlen_at.txt (per-user, M lines)\n";
    cout << "#  - baseline_curve.csv\n";
    cout << "#  - rate_limited_estimated_crackedpw_varlen_at.txt (per-user estimated, M lines, -1 = not within cap)\n";
